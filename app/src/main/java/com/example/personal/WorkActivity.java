...
public class WorkActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    public RecyclerView recyclerwork ;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        recyclerwork = findViewById(R.id.RecyclerView_work);
        recyclerwork.setHasFixedSize(true);
        Manager = new LinearLayoutManager(this);
        Adapter = new MyThirdAdapter(items);
        recyclerwork.setLayoutManager(Manager);
        recyclerwork.setAdapter(Adapter);
        Adapter.setOnItemClickListener(new MyThirdAdapter.OnItemClickListener() {
            @Override
            public void onItemclick(int position) {
                Toast.makeText(WorkActivity.this,""+position,Toast.LENGTH_SHORT).show();
                String pose;
                Intent General = new Intent(WorkActivity.this,GeneralActivity.class) ;
                String name = "Work";
            ...
            public void onDoneclick(int position) {
                ...
                if(x>(lvl +1)* 10){
                    lvl++;
                    Toast.makeText(WorkActivity.this, "Level Up", Toast.LENGTH_SHORT).show();
                    x= x - (lvl +1) * 10;
                    wd=1;
                    if(x<0){
                        x = x*(-1);
                ...
                //Toast.makeText(WorkActivity.this,"Done Clicked..and subject = ",Toast.LENGTH_SHORT).show();
            }
            @Override
            public void onCheckclick(int position) {
                ...
                items.remove(position);
                Adapter.notifyDataSetChanged();
                //Toast.makeText(WorkActivity.this,"Check Clicked..",Toast.LENGTH_SHORT).show();
    ...
    public boolean onNavigationItemSelected(@NonNull MenuItem menuItem) {
        switch (menuItem.getItemId()) {
            case R.id.nav_general:
                Intent generalIntent = new Intent(WorkActivity.this, GeneralActivityM.class);
            ...
            case R.id.nav_friends:
                Intent Social = new Intent(WorkActivity.this, SocialActivity.class);
                startActivity(Social);
                break;
            case R.id.nav_events:
                Intent eventIntent = new Intent(WorkActivity.this, EventsActivity.class);
    ...
    private void getData(DatabaseReference dbf, String id){
        dbf.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if(flag == 0){
                    for(int i=0;i<10;i++){
                        String m = snapshot.child("Category").child(id).child("Work").child(String.valueOf(i)).child("Subject").getValue(String.class);
                        if(m.equals(""))
                            i++ ;
                        else
                        {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if(flagNCS == 0){
                    ...
                    for(i=0,NumberChekersub = "0";i<10;i++){
                        String nn = snapshot.child("Category").child(uid).child("Work").child(String.valueOf(i)).child("Subject").getValue(String.class);
                        ...
                        if(nn.equals(sub)){
                            ...
                            if(flagNCS == 0){
                                ...
                                FirebaseDatabase.getInstance().getReference().child("Category").child(uid).child("Work").child(NumberChekersub).child("Date")
                                        .setValue("");
                                FirebaseDatabase.getInstance().getReference().child("Category").child(uid).child("Work").child(NumberChekersub).child("Description")
                                        .setValue("");
                                FirebaseDatabase.getInstance().getReference().child("Category").child(uid).child("Work").child(NumberChekersub).child("Subject")
                                        .setValue("");
                                FirebaseDatabase.getInstance().getReference().child("Category").child(uid).child("Work").child(NumberChekersub).child("Time")
                                        .setValue("");
